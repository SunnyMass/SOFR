...
class DMC(CompressionModel):
    ...
    def warp(self, img, flow):
        ...
        return output
    def motion_estimation_and_mv_encoding(self, x_s,x_d,ref_x_s,ref_x_d, dpb, mv_y_q_enc,q_index,frame_idx):
        est_mv_s = self.optimize_down_sampling_ratio_encode(x_s, ref_x_s,q_index)
        est_mv_d = self.optic_flow(x_d, ref_x_d)
        mv_y = self.mv_encoder(est_mv_s, est_mv_d, dpb["ref_mv_feature"], mv_y_q_enc)
        return mv_y
    
    def optimize_down_sampling_ratio_encode(self, x, ref_x_s, q_index):
        ...
        ref_c = F.interpolate(ref_x_s, scale_factor=1/scale_coarse, mode="bilinear", antialias=True)
        ...
        # Fine Flow
        x_pad, h, w = pad_img(x)
        ref_pad, _, _ = pad_img(ref_x_s)
        
        mv_f_pad = self.optic_flow(x_pad, ref_pad)
        mv_fine = mv_f_pad[:, :, :h, :w]

        # Reliability check
        x_hat_c_check = self.warp(ref_x_s, mv_coarse)
        x_hat_f_check = self.warp(ref_x_s, mv_fine)
        ...
        # Fusion Mask
        x_hat_coarse = self.warp(ref_x_s, mv_coarse)
        err_coarse = torch.mean(torch.abs(x - x_hat_coarse), dim=1, keepdim=True)
        
        x_hat_fine = self.warp(ref_x_s, mv_fine)
    ...
    def compress(self, x, dpb, q_in_ckpt, q_index, frame_idx):
        ...
        mv_y = self.motion_estimation_and_mv_encoding(x_s,x_d,ref_x_s,ref_x_d, dpb, mv_y_q_enc,q_index,frame_idx)
