...
class MvEnc(nn.Module):
    ...
    def forward(self, x, context, quant_step):
        ...
        out = out * quant_step
        out = self.enc_2(out)
        if context is None:# or context.shape!=out.shape:
...
class DMC(CompressionModel):
    ...
    def mv_prior_param_decoder(self, mv_z_hat, dpb, slice_shape=None):
        ...
        if ref_mv_y is None: #or mv_params.shape != ref_mv_y.shape:
    ...
    def encode(self, x, dpb, q_index, fa_idx, sps_id=0, output_file=None, frame_number=0, args=None):
        # print("encode")
        # pic_width and pic_height may be different from x's size. x here is after padding
        # x_hat has the same size with x
        dpb["ref_down_ratio"] = 1
        if output_file is None:
            encoded = self.forward_one_frame(x, dpb, q_index=q_index, fa_idx=fa_idx, video_name=args['seq'], frame_number=frame_number)
            result = {
                "dpb": encoded['dpb'],
