...
# Line 135-164 is modified by ByteDance
class MultiScaleKeyContextFusion(nn.Module):
    def __init__(self, inplace=False):
        super().__init__()
        self.conv3_up = subpel_conv3x3(g_ch_4x * 2, g_ch_2x, 2)
        self.res_block3_up = ResBlock(g_ch_2x, inplace=inplace)
        self.conv3_out = nn.Conv2d(g_ch_4x * 2, g_ch_4x, 3, padding=1)
        self.res_block3_out = ResBlock(g_ch_4x, inplace=inplace)
        self.conv2_up = subpel_conv3x3(g_ch_2x * 3, g_ch_1x, 2)
        self.res_block2_up = ResBlock(g_ch_1x, inplace=inplace)
        self.conv2_out = nn.Conv2d(g_ch_2x * 3, g_ch_2x, 3, padding=1)
        self.res_block2_out = ResBlock(g_ch_2x, inplace=inplace)
        self.conv1_out = nn.Conv2d(g_ch_1x * 3, g_ch_1x, 3, padding=1)
        self.res_block1_out = ResBlock(g_ch_1x, inplace=inplace)

    def forward(self, context1, context2, context3, key1, key2, key3):
        context3_up = self.conv3_up(torch.cat((context3, key3), dim=1))
        context3_up = self.res_block3_up(context3_up)
        context3_out = self.conv3_out(torch.cat((context3, key3), dim=1))
        context3_out = self.res_block3_out(context3_out)
        context2_up = self.conv2_up(torch.cat((context3_up, context2, key2), dim=1))
        context2_up = self.res_block2_up(context2_up)
        context2_out = self.conv2_out(torch.cat((context3_up, context2, key2), dim=1))
        context2_out = self.res_block2_out(context2_out)
        context1_out = self.conv1_out(torch.cat((context2_up, context1, key1), dim=1))
        context1_out = self.res_block1_out(context1_out)
        context1 = context1 + context1_out
        context2 = context2 + context2_out
        context3 = context3 + context3_out
...
class DMC(CompressionModel):
    def __init__(self, anchor_num=4, ec_thread=False, stream_part=1, inplace=False, layer_num=3):
        ...
        # Line 290 is modified by ByteDance
        self.key_align = OffsetDiversity(aux_feature_num=g_ch_1x+3+2*4, inplace=inplace)
        ...
        # Line 319 is modified by ByteDance
        self.feature_key_adaptor = nn.ModuleList([nn.Conv2d(g_ch_1x, g_ch_1x, 1) for _ in range(3)])
        self.feature_extractor = FeatureExtractor(inplace=inplace)
        # Line 322 is modified by ByteDance
        self.context_fusion_net = MultiScaleKeyContextFusion(inplace=inplace)

        self.contextual_encoder = ContextualEncoder(inplace=inplace)
        # Line 326-331 is modified by ByteDance
        self.forward_feature_extractor = nn.Sequential(
            nn.Conv2d(3, g_ch_1x, 3, stride=1, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=inplace),
            nn.Conv2d(g_ch_1x, g_ch_1x, 3, stride=1, padding=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=inplace),
        )
        ...
        # Line 375 is modified by ByteDance
        self.y_q_layer_enc = nn.Parameter(torch.ones((layer_num, 1, 1, 1)))
        ...
        # Line 380 is modified by ByteDance
        self.y_q_layer_dec = nn.Parameter(torch.ones((layer_num, 1, 1, 1)))
    ...
    # Line 395-404 is modified by ByteDance
    def multi_scale_key_feature_extractor(self, dpb, index):
        if dpb["ref_feature"] is None:
            feature = self.feature_adaptor_I(dpb["ref_frame"])
        else:
            index = index % 4
            index_map = [0, 1, 0, 2]
            index = index_map[index]
            feature = self.feature_key_adaptor[index](dpb["key_feature"])
    ...
    def get_recon_and_feature(self, y_hat, context1, context2, context3, y_q_dec):
        ...
        # Line 479 is modified by ByteDance
    ...
    def warp(self, img, flow):
        ...
        return output
    def motion_estimation_and_mv_encoding(self, x, dpb, mv_y_q_enc, q_index):
        est_mv = self.optimize_down_sampling_ratio_encode(x, dpb,q_index)
        ref_mv_feature = dpb["ref_mv_feature"]
        mv_y = self.mv_encoder(est_mv, ref_mv_feature, mv_y_q_enc)
        return mv_y

    def optimize_down_sampling_ratio_encode(self, x, dpb, q_index):
        if isinstance(q_index, torch.Tensor):
            q_val = int(q_index.item())
        else:
            q_val = int(q_index) if q_index is not None else 0
        ...
        def pad_img(img, p=8):
            ...
            pad_b = (p - h % p) % p
            pad_r = (p - w % p) % p
            return F.pad(img, (0, pad_r, 0, pad_b)), h, w

        # Coarse Flow
        x_c = F.interpolate(x, scale_factor=1/scale_coarse, mode="bilinear", antialias=True)
        ref_c = F.interpolate(dpb["ref_frame"], scale_factor=1/scale_coarse, mode="bilinear", antialias=True)
        
        x_c_pad, h_c, w_c = pad_img(x_c)
        ref_c_pad, _, _ = pad_img(ref_c)
        
        mv_c_pad = self.optic_flow(x_c_pad, ref_c_pad)
        mv_c = mv_c_pad[:, :, :h_c, :w_c]
        
        mv_coarse = F.interpolate(mv_c, size=(x.shape[2], x.shape[3]), mode="bilinear", antialias=True) * scale_coarse
        
        # Fine Flow
        x_pad, h, w = pad_img(x)
        ref_pad, _, _ = pad_img(dpb["ref_frame"])
        
        mv_f_pad = self.optic_flow(x_pad, ref_pad)
        mv_fine = mv_f_pad[:, :, :h, :w]

        # Reliability check
        x_hat_c_check = self.warp(dpb["ref_frame"], mv_coarse)
        x_hat_f_check = self.warp(dpb["ref_frame"], mv_fine)
        
        sad_coarse = torch.mean(torch.abs(x - x_hat_c_check))
        sad_fine   = torch.mean(torch.abs(x - x_hat_f_check))
        
        if sad_coarse >= sad_fine:
            mv_coarse = mv_fine

        # Fusion Mask
        x_hat_coarse = self.warp(dpb["ref_frame"], mv_coarse)
        err_coarse = torch.mean(torch.abs(x - x_hat_coarse), dim=1, keepdim=True)
        
        x_hat_fine = self.warp(dpb["ref_frame"], mv_fine)
        err_fine = torch.mean(torch.abs(x - x_hat_fine), dim=1, keepdim=True)
        
        diff = err_coarse - (err_fine + bias)
        
        sharpness = 10.0
        mask = torch.sigmoid(diff * sharpness) 
        
        mask = F.avg_pool2d(mask, kernel_size=5, stride=1, padding=2)
        best_est_mv = mask * mv_fine + (1 - mask) * mv_coarse
    ...
    def compress(self, x, x_next, dpb, q_in_ckpt, q_index, frame_idx):
        mv_y_q_enc, mv_y_q_dec, y_q_enc, y_q_dec = self.get_q_for_inference(q_in_ckpt, q_index, frame_idx)
        mv_y = self.motion_estimation_and_mv_encoding(x, dpb, mv_y_q_enc,q_index)
        ...
        # Line 559-560 is modified by ByteDance
        key_feature = dpb["key_feature"]
        key_mvs = dpb["key_mvs"]
        ...
        result = {
            "dpb": {
                ...
                "ref_y": y_hat,
                "ref_mv_y": mv_y_hat,
                # Line 570-571 is modified by ByteDance
    ...
    def decompress(self, dpb, string, height, width, q_in_ckpt, q_index, frame_idx):
        _, mv_y_q_dec, _, y_q_dec = self.get_q_for_inference(q_in_ckpt, q_index, frame_idx)
        ...
        # Line 607-613 is modified by ByteDance
        if frame_idx%4 == 1:
            key_feature = feature
            key_mvs = []
        else:
        ...
        return {
            "dpb": {
                ...
                "ref_y": y_hat,
                "ref_mv_y": mv_y_hat,
                # Line 623-624 is modified by ByteDance
                "key_feature": key_feature,
                "key_mvs": key_mvs,
            },
        }

    def encode_decode(self, x, dpb, q_in_ckpt, q_index, x_next=None, output_path=None,
                      pic_width=None, pic_height=None, frame_idx=0):
        if output_path is not None:
            device = x.device
            torch.cuda.synchronize(device=device)
            t0 = time.time()
            # Line 635 is modified by ByteDance
            encoded = self.compress(x, x_next, dpb, q_in_ckpt, q_index, frame_idx)
        ...
        raise ValueError('Unknown bitstream path, please specify!')
